...
var codexProfile = Profile{BinaryName: "tmux-msg-codex", DisplayLabel: "Codex"}
...
func TestUsageText_ThreadsProfile(t *testing.T) {
	t.Run("codex profile", func(t *testing.T) {
		...
		for _, want := range []string{
			"usage: tmux-msg-codex <subcommand>",
	...
	t.Run("claude default unchanged", func(t *testing.T) {
		got := usageText()
		for _, want := range []string{
			"usage: tmux-msg-claude <subcommand>",
...
func TestSubcommandUsageHint_ThreadsProfile(t *testing.T) {
	t.Run("codex profile", func(t *testing.T) {
		...
		got := stderr.String()
		if !strings.Contains(got, "usage: tmux-msg-codex config") {
			t.Errorf("config usage hint not threaded with codex binary; got %q", got)
		}
		if strings.Contains(got, "tmux-msg-claude") {
			t.Errorf("config usage hint still names tmux-msg-claude under codex profile; got %q", got)
		}
	})
	...
	t.Run("claude default unchanged", func(t *testing.T) {
		var stderr bytes.Buffer
		runConfigCLI(nil, &bytes.Buffer{}, &stderr)
		if got := stderr.String(); !strings.Contains(got, "usage: tmux-msg-claude config") {
