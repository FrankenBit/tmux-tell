...
func TestClassifyDoctorProcs(t *testing.T) {
	bindings := []dbBinding{
		...
		{PID: 1, BinaryPath: "/usr/local/bin/tmux-msg-claude", DBPath: testCanonPath, DBInode: testCanonInode},
		// orphan: open handle, dirent removed (the deploy-mv inode)
		{PID: 2, BinaryPath: "/usr/local/bin/tmux-msg-claude", DBPath: "/var/lib/tmux-msg/messages.db", DBInode: 50, DBDeleted: true},
		// divergent inode: bound to a different live DB
		{PID: 3, BinaryPath: "/usr/local/bin/tmux-msg-claude", DBPath: "/tmp/other/messages.db", DBInode: 77},
		// stale binary: DB matches canonical, but running since-replaced code
		{PID: 4, BinaryPath: "/usr/local/bin/tmux-msg-claude (deleted)", DBPath: testCanonPath, DBInode: testCanonInode},
		// no open DB handle: reported, not counted divergent
		{PID: 5, BinaryPath: "/usr/local/bin/tmux-msg-claude", Note: "no open *.db handle (in-memory store, or DB not yet opened)"},
	}
...
func TestClassifyDoctorProcs_AllCanonicalNoDivergence(t *testing.T) {
	bindings := []dbBinding{
		{PID: 1, BinaryPath: "/usr/local/bin/tmux-msg-claude", DBPath: testCanonPath, DBInode: testCanonInode},
		{PID: 2, BinaryPath: "/usr/local/bin/tmux-msg-claude", DBPath: testCanonPath, DBInode: testCanonInode},
	}
	rep := classifyDoctorProcs(bindings, testCanonPath, testCanonInode)
	if rep.Divergent {
...
func TestClassifyDoctorProcs_CanonicalAbsentCannotCompare(t *testing.T) {
	bindings := []dbBinding{
		{PID: 1, BinaryPath: "/usr/local/bin/tmux-msg-claude", DBPath: "/somewhere/messages.db", DBInode: 42},
	}
	...
	orphan := classifyDoctorProcs([]dbBinding{
		{PID: 2, BinaryPath: "/usr/local/bin/tmux-msg-claude", DBPath: "/gone/messages.db", DBInode: 9, DBDeleted: true},
	}, testCanonPath, 0)
	if !orphan.Divergent {
