	...
	srv.RegisterTool("tmux-tell.send",
		...
		json.RawMessage(`{
			...
			"properties": {
				...
				"deliver_after":     {"type": "string", "description": "Defer delivery until a trigger fires (#227): the message is STAGED (not queued) and delivers only after the trigger. Accepts \"resume\" — post-compaction self-handoff: stage orientation text with deliver_after=\"resume\" so it lands in the freshly-resumed context instead of being absorbed by the summarizer. It auto-promotes (#843) when the mailman delivers a bus session reset (/compact or /clear) to you, so a compact triggered via tmux-tell.control needs no flush; call tmux-tell.flush_deferred{trigger:\"resume\"} in your resume routine when you compacted WITHOUT the bus (a /compact typed straight into your pane leaves no control row for the mailman to see) — or \"register\" (#258a): a spawn-die session bridge addressed to another agent (\"remember this for its next dispatch\"); it auto-promotes when that agent next (re)registers, no explicit flush needed. Single-recipient only. The response carries deliver_after to confirm staging."},
	...
	srv.RegisterTool("tmux-tell.control",
		...
		json.RawMessage(`{
			...
			"properties": {
				...
				"resume_with": {"type": "string", "description": "Optional continuation prompt delivered after the compact (/compact) settles. Only valid with command=compact on self-invocation."},
				...
				"force_rate_limited": {"type": "boolean", "description": "Bypass the recipient's rate-limit / usage-limit defer for this control macro, delivering even when the pane shows a rate-/usage-limit banner (#573, control arm of #558). Applies to BOTH rows of the restart / compact+resume / clear+rename macros. Does NOT bypass copy-mode / popup / unknown / compaction paste-safety."}
			},
			"required": ["to", "command"]
		}`),
		mcpControlHandler(s))
