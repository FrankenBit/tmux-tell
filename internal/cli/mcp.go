...
func newMCPServer(s *store.Store) *mcp.Server {
	...
	srv.RegisterTool("tmux-msg.ping",
		"Substrate-only reachability probe (#144): is the recipient's mailman daemon up and its pane reachable? Queues a kind=ping row that the mailman picks up (proving the daemon is alive) and answers via substrate-health checks (agent registered, pane live) — it does NOT paste into the recipient's pane or load their context, so it's safe for runbook verification and post-restart sanity. Returns {ok, agent, id, state, elapsed_ms}: state is `delivered` (reachable), `failed` (registered but unreachable — pane gone), or `timeout` (no mailman answered in time). On UNREACHABLE (ok=false) the response also carries a structured `reason` (#358) — one of `pane_dead` / `mailman_down` / `stuck` / `blocked_delivery` / `backlog_draining` — plus an `evidence` block {mailman_active, queue_depth, current_state, stuck_reason?} so the caller can route recovery: mailman_down → start the daemon, blocked_delivery → investigate the observe-gate, backlog_draining → wait, stuck → `register --force`, pane_dead → re-discover. Pinging a non-registered agent fails loud.",
		json.RawMessage(`{
			"type": "object",
			"properties": {
