...
When a chamber's `/compact` takes longer than the stability ceiling to settle, the mailman delivered the promoted resume into a still-redrawing pane. Every submit-Enter was eaten by the redraw. The delivery is now marked `PostCompactSettle` and runs an extended verify budget so the settle-gated resubmit fires once the pane finally settles. The flag is armed only on a stability-ceiling blow and consumed once by the next delivery, so general-work delivery is byte-identical — mid-work steering messages still land in the composer during active work.
