...
Post-compaction resume messages no longer stall pasted-but-unsubmitted after `#866` — the verify no longer mistakes a busy-restoring pane's empty composer for a successful submit (`#873`).

`#866` extended the post-`/compact` verify budget so the settle-gated resubmit could fire once the pane settled, but a race upstream of it kept firing: right after `/compact`, the TUI has been sent the pasted bytes but has not yet *drawn* them, so the composer reads empty. The cursor-anchored "input cleared" check read that pre-render empty as a submission and returned verified in ~1-4s — before the paste ever appeared — so the extended budget never engaged, the eaten submit-Enter left the paste sitting, and the operator had to press Enter by hand (measured n=5 and n=6 on post-`/compact` wake).

The verify now refuses to credit a cleared composer as submitted until it has seen our paste *present* in the live input at an earlier poll — the non-empty→empty transition is the evidence the paste actually landed and then submitted. A clear with no prior present observation is the pre-render empty, not our submission, so the loop keeps polling; once the paste renders, the existing settle-gated resubmit submits it. Scoped to the post-compact settle path, so normal deliveries are unchanged.
...
When a chamber's `/compact` takes longer than the stability ceiling to settle, the mailman delivered the promoted resume into a still-redrawing pane. Every submit-Enter was eaten by the redraw. The delivery is now marked `PostCompactSettle` and runs an extended verify budget so the settle-gated resubmit fires once the pane finally settles. The flag is armed only on a stability-ceiling blow and consumed once by the next delivery. General-work delivery stays byte-identical — mid-work steering messages still land in the composer during active work.

The class hid through the v0.35.0 fix arc: the delivery verifier keyed on "did the composer empty", a signal blind to whether operator or mailman pressed Enter. Every rescue recorded a clean `verified=1`; the `delivered_in_input_box` signal never fired. A post-compact clear that cannot be attributed to a settled-frame Enter of the mailman's is now reported as the new `delivered_externally_submitted` delivery state (metric + WARN). The failure is finally measurable rather than counted as a mailman success.
...
Added a `StateErrored` pane classification that detects Claude Code's terminal API-error chrome (`● API Error: <code> …`). Previously the mailman read such a pane as Idle and pasted into the dead turn as ghost text. Detection is live-scoped: the error line must sit in the current-turn region and carry a status code, so scrollback quotes cannot fire it. It is suppressed while `esc to interrupt` shows, so an active 529-retry stays Working and only a terminal error classifies Errored. `StateErrored` is paste-unsafe (`IsPasteUnsafeForced`), so the mailman aborts the delivery. Claude-only; codex parks it via an empty `APIErrorMarker`.
...
Fixed Claude paste deliveries that landed in the composer but never submitted, leaving the operator to press Enter by hand. The `#401`/`#674` resubmit machinery was unreachable on Claude: it gated on the empty `PasteCollapseMarker`, so the "still unsubmitted?" check was always false. A new `PasteEvidenceMarker` field splits that question from body-normalization: Claude supplies `[Pasted text`, and codex is unchanged. The same predicate now also guards the whole-pane fallback in `deliverySubmitted`. Scope is Linux-rendered Claude; the ASCII `> ` sentinel path gains no resubmit. Measurement refuted the paste-size hypothesis, so the fix is cause-agnostic recovery of an objectively-unsubmitted paste.

`deliver_after=resume` messages now fire on their own after a bus-delivered session reset, instead of sitting in `deferred` forever. Previously only an explicit `flush_deferred{trigger:"resume"}` promoted them: the `register` trigger got an auto-fire in #258a, but `resume` never did, so staged self-handoffs rotted (seven across two chambers). The mailman now promotes an agent's `resume`-staged rows when it delivers a session reset (`/compact` or `/clear`), ordered ahead of traffic that arrived while away. It is deliberately not gated on `--post-compact-pause`. A `/compact` typed directly into a pane leaves no control row, so that path still needs the explicit flush.

`deliver_after=resume` rows now also auto-fire on a **self-typed** `/compact`, not only a bus-delivered one. #843 covered a reset that arrived as a control row; a directly-typed `/compact` leaves none, so those staged rows still rotted — the 4-of-7 residual #843 disclosed. The mailman now edge-detects the `last_self_compact_at` signal against a second watermark (`resume_promoted_at`) and promotes the resume rows at that edge. The distinct watermark keeps the promote and the respawn counter from consuming each other's edge. It ships **dormant** until the `PostCompact` hook is wired fleet-wide (alcatraz-infra#391), then activates on the next deploy.
