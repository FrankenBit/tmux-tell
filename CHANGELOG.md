...
Added a `reap` verb that dead-letters undeliverable queued fossils — messages to an unreachable recipient with no live pane. These sit `queued` forever, and because the per-recipient/per-sender caps count only `queued` rows, enough of them wedge a channel. `reap` transitions fossils to `failed` with a dead-letter reason (audit-preserving, not a delete), freeing the cap slot. It requires exactly one of `--dry-run` or `--confirm`, takes `--older-than` (default 7d) and an optional `--agent` scope. Crucially it is scoped by recipient *liveness*, never age: a recipient holding a live-pane registration is never reaped, so its mail is safe.

Added an opt-in observer-hosted auto-reap sweep — phase 2 of the undeliverable-fossil work (#836), sibling to the #726 `reap` verb. With `auto-reap-enabled = true` in `[defaults]`, the fleet mailman observer periodically dead-letters fossils across all chambers, using the same recipient-liveness predicate as `reap`. It is **off by default** (the reap is destructive-adjacent) and tunable via `auto-reap-interval` (6h) and `auto-reap-older-than` (7d). When `observer-metrics-addr` is set, the observer exposes a Prometheus `/metrics` endpoint publishing a `tmux_tell_reaped_total{reason=...}` counter. The observer runs whenever *either* alerting or auto-reap is enabled — the jobs are independent, so auto-reap works without a configured `mailman-alert-to`.
...
Fixed the cursor-less classifier fallback returning `unknown` on healthy codex panes with rendered turns. Codex prefixes every turn with its `› ` sentinel, so a pane with any history carries multiple sentinel rows. The fallback disqualified the whole pane once any sentinel row had content past it, so the mailman's pre-paste gate refused delivery in a loop. It now anchors on the bottom-most sentinel row — the composer — so transcript rows above no longer poison the classification. Claude's chrome doesn't repeat the sentinel, so Claude panes are unaffected. Bug 2 of #756; Bug 1 is tracked as #827.
...
Fix `tmux-tell.agent_state` MCP tool (and the CLI `state --agent NAME` sibling) false-negativing on cross-adapter probes. A Claude-adapter binary probing a codex chamber previously classified it as `unknown`, because it searched for the caller's `❯ ` sentinel in a `› ` pane. The classifier now routes the pane-observation profile per TARGET agent via the stored `provider` column, so probes across adapter boundaries classify against the target's sentinels/markers. Additive: agents without a stamped provider fall back to the caller's active profile — pre-fix behavior preserved.

Fixed `install.sh` leaving the mailman observer (`tmux-tell-mailman-observer.service`, #808) on the pre-install binary after a redeploy. The install step only ran `enable --now`, a no-op on a running unit, so the observer kept the deleted inode while every mailman was restarted. The stale observer then tripped the doctor's real-divergence check (exit 69, mailman-stale) and hard-failed the deploy — as happened on v0.33.1. `install.sh` now `enable`s **and** `restart`s the observer every install, mirroring the `restart-mailmen` contract (#436). The restart is fatal by default (a warning under `--allow-stale-mailmen`), so a failed observer restart fails the deploy instead of greening a stale binary.
...
Added a delivery-freshness alert. A queued message that sits undelivered past `--mailman-stale-threshold` (default 10m) triggers a one-shot notice to a configured conductor agent, surfacing stalled panes instead of silently swallowing deliveries. Known hold states (rate/usage-limit, awaiting-operator, copy-mode, compaction-rest) suppress the alert. Off by default; set `mailman-alert-to` (flag or per-agent config) to activate.

Added an independent systemd mailman observer. It sweeps expected per-agent mailman units every 30 seconds. When an enabled unit stays inactive or enters a restart loop, it sends a one-shot conductor notice — catching OOM/crash failures the mailman's self-observed freshness alert cannot detect.
...
Fixed the pane-state classifier misreading operator drafts as `working` (paste-safe), which let the mailman clobber the half-typed input. Each keystroke repainted the input row, so the frame-change precedence fired before the cursor-position branch could see the operator was drafting. The classifier now lets "cursor **past** the prompt sentinel" win over frame-change → `awaiting-operator` (paste-unsafe), deferring delivery until the draft is sent or cleared. Streaming and spinner busy-states keep the cursor **at** the sentinel and still classify `working`, so mid-turn delivery is unaffected (156/156 busy frame-changes measured on the live Claude adapter).

Fixed `tmux-tell.register` MCP tool silently rewriting `pane_id` from `$TMUX_PANE` in acting-on-behalf and post-crash scenarios (mirror of CLI fix in #570/#403). Added a `keep_pane` input field to update non-pane fields without touching `pane_id`. Added divergence detection: refuses when stored `pane_id` differs from `$TMUX_PANE` and neither `pane` nor `keep_pane` was passed.
...
Deploy no longer false-positives on idle-at-prompt chambers with stale-binary agent-side MCPs. `install.sh --no-bootstrap` now autonomously fires `refresh-all-mcps` when the operator's tmux server is reachable, non-fatal on partial failure. `doctor --allow-active-agents` splits its exit codes: **71** (`exitDoctorIdleStaleOnly`) when all divergences are the idle-stale class closable by `refresh-all-mcps`. Exit **69** covers real divergences that no refresh can close: SYNC db-state, mailman-stale, orphan MCP. `.forgejo/workflows/deploy.yml` post-smoke case-switches: `0` clean, `71` `::warning::`, anything else hard-fail. Closes #416, restoring the hard-fail-on-real-divergence goal that #791 partially delivered.

Fixed `ping` failing with an opaque SQLite error under sandbox FS scope (readonly database). When `store.Open` hits a read-only error, the output now names the sandbox as the likely cause and surfaces two remediation options. Use the `tmux-tell.ping` MCP tool (write access, persistent process outside the sandbox), or run `ping` outside the sandbox with `--db <path>`. Non-sandbox failures (permission denied, bad path) preserve their original error text unchanged.

Fixed a data race in `internal/notify` between `fan()` and Watch's cleanup goroutine. `fan()` was copying only the slice HEADER of `w.subs[key]` under the mutex, then iterating the shared backing array lock-free — while cleanup shifted elements in place via `append(subs[:i], subs[i+1:]...)`. Fix: `fan()` now deep-copies the subscriber slice under the mutex, giving the send loop an immutable snapshot. Send loop stays lock-free (a slow consumer still can't block a Watch or Close). CI is unaffected — `-race` needs cgo and lives only in the documented local pre-commit step (see #818).
...
- **respawn**: self-compact detection via post-compaction hook (#285 PR2)
- **respawn**: bounded post-shrink chamber respawn, bus-clear trigger (#285 PR1)
- **debug**: TMUX_TELL_DEBUG gate + ping evidence-gather first consumer (#681, #365)
...
- **ci**: re-pin release-toolkit v0.18.0 → v0.20.0-rc.2
