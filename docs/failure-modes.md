...
tmux-tell shipped end-of-MVP on 2026-05-29 and within 48h
...
`tmux-tell-claude health` or in `status`, would have surfaced incidents
...
- [x] [#42 — `tmux-tell-claude health` subcommand](https://git.frankenbit.de/frankenbit/tmux-tell/issues/42)
...
- [x] [#39 — deliver-time histogram per recipient (95th/99th)](https://git.frankenbit.de/frankenbit/tmux-tell/issues/39)
...
- [x] [#40 — per-verdict count from `WaitForQuietPane`](https://git.frankenbit.de/frankenbit/tmux-tell/issues/40)
...
- [x] [#41 — per-mailman crash counter in status](https://git.frankenbit.de/frankenbit/tmux-tell/issues/41)
...
- **Location**: in-package, in dedicated `pin_test.go` files. One
  ...
  (`cmd/tmux-tell-claude/pin_test.go`, `internal/store/pin_test.go`,
...
- Comparing tmux-tell's incident rate to industry benchmarks.
