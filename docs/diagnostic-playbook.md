...
[#63](https://git.frankenbit.de/frankenbit/tmux-tell/issues/63) covers
...
generating a hypothesis (*"tmux-tell dropped it"*), not a verified
...
sqlite3 ~/.local/share/tmux-tell/messages.db -header -column "
...
```bash
journalctl --user-unit=tmux-tell-claude-mailman@<receiver> \
  --since="<T-2min local>" \
  --until="<T+2min local>" \
  --no-pager
```
...
## Watching it happen live — `tmux-tell-claude tail`
...
tmux-tell-claude tail --from bosun --to surveyor

# only failures / notices, across everyone
tmux-tell-claude tail --kind delivery_failure_notice
tmux-tell-claude tail --state failed
```
...
When a `tmux-tell-claude send` command reports `delivery: state=failed` and the
mailman log shows
...
- `tmux-tell-claude send …` returns `state: failed` with `error: drift_detected_unrecoverable`.
...
1. **Match the name (recommended).** Re-register with the pane's self-declared
   ...
   tmux-tell-claude register --name <pane-title-lowercased> --pane <pane-id> --force
   ```
...
2. **Override with `--drift-soft-fail`** (deliberate experiments / atypical
   ...
   ```bash
   tmux-tell-claude serve --agent <name> --drift-soft-fail
   ```
...
A distinct failure mode surfaces when a Codex agent calls `tmux-tell.send` via the MCP
server path (`[mcp_servers.tmux-tell]` in `~/.codex/config.toml`): the call fails or reports
...
CLI path (`tmux-tell-codex send …`) and the hook-context path run in the operator's shell
where both variables are available; only the MCP server spawn is isolated.

**Symptom.** `tmux-tell.send` calls via the MCP server return a sender-resolution error or
send with `from = ""` instead of the agent's registered name.

**Fix.** Add an `env` table to the `[mcp_servers.tmux-tell]` stanza in
`~/.codex/config.toml`:
...
[`cmd/tmux-tell-codex/README.md`](../cmd/tmux-tell-codex/README.md) §MCP server for the full
wiring recipe.

## Post-deploy MCP-binding divergence

A deploy that moves the DB file (e.g. the #308 path move from
`/var/lib/tmux-msg/` to `~/.local/share/tmux-tell/`) but does **not** restart the
...
**Symptom.** A sender's `tmux-tell.agents` reports `queued: N` for a recipient,
...
`tmux-tell-claude` process, reads each one's *actual* open DB handle from
`/proc/PID/fd` (inode and all), and flags any that diverge from the canonical
DB:

```bash
tmux-tell-claude doctor          # exits non-zero on any divergence
tmux-tell-claude doctor --format json
```

```
CANONICAL	/home/alex/.local/share/tmux-tell/messages.db (inode 1895479)
✓ PID 326865  binary=/usr/local/bin/tmux-tell-claude  db=…/messages.db (inode 1895479)  — canonical
✗ PID 12381   binary=/usr/local/bin/tmux-tell-claude (deleted)  db=/var/lib/tmux-msg/messages.db (inode 12345) [file removed]  — orphan DB inode — file removed; writes invisible to mailmen on the canonical path
DIVERGENCE: one or more processes hold a DB binding invisible to mailmen on the canonical path. Run `tmux-tell-claude refresh-all-mcps` to restart MCP servers (and check for stale mailmen).
```

**Ask one process directly** — `tmux-tell.whoami_db` (MCP tool) returns the
...
**Confirm ground truth** — `tmux-tell-claude track <id> --canonical` opens the
...
**Fix.** `tmux-tell-claude refresh-all-mcps` restarts the MCP servers so they
...
[#343](https://git.frankenbit.de/frankenbit/tmux-tell/issues/343); `doctor` is
the *detection* half of that defense-in-depth pair ([#348](https://git.frankenbit.de/frankenbit/tmux-tell/issues/348)).
...
Or use `tmux-tell-claude track <id>` if you already know the message id.
...
```bash
systemctl --user restart tmux-tell-claude-mailman@<agent>
# or, for Codex:
systemctl --user restart tmux-tell-codex-mailman@<agent>
```
...
env vars — see [#356](https://git.frankenbit.de/frankenbit/tmux-tell/issues/356)),
...
1. Fix the underlying registration (`register --start-mailman` or
   ...
   tmux-tell-claude-mailman@<agent>`). The unit start triggers recovery.
